...
const batchDelete = async () => {
  ...
  const nModified = await proxy.$api.userDel({userIds: selectUserIds.value})
  if (nModified > 0) {
    proxy.$message.success('删除成功')
    await getUserList()
  }
...
const handleSelectionChange = (list) => {
  list.forEach((item) => {
    selectUserIds.value.push(item.userId)
  })
}
//控制表单显示
...
const submitUserCreateForm = () => {
  proxy.$refs.userCreateFormRef.validate(async (valid) => {
    if (valid) {
      ...
      let res = await proxy.$api.createUser(params)
      if (res) {
        userFormVisible.value = false
